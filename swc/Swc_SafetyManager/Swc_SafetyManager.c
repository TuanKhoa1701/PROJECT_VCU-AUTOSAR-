...
#define SAFETY_TIMEOUT_TICKS \
  ((uint8)((SAFETY_TIMEOUT_MS + (SAFETY_TASK_PERIOD_MS - 1u)) / SAFETY_TASK_PERIOD_MS))
 ...
 */
static uint8 Safety_ApplyBrakeOverride(uint8 throttlePct, boolean brake)
{
...
void Swc_SafetyManager_Run10ms(void)
{
  ...
  s_safety.missPedal = (havePedal ? 0u : (uint8)((s_safety.missPedal < 0xFFu) ? s_safety.missPedal + 1u : 0xFFu));
  s_safety.missBrake = (haveBrake ? 0u : (uint8)((s_safety.missBrake < 0xFFu) ? s_safety.missBrake + 1u : 0xFFu));
  s_safety.missGear  = (haveGear  ? 0u : (uint8)((s_safety.missGear  < 0xFFu) ? s_safety.missGear  + 1u : 0xFFu));
  s_safety.missMode  = (haveMode  ? 0u : (uint8)((s_safety.missMode  < 0xFFu) ? s_safety.missMode  + 1u : 0xFFu));
  ...
  uint8       reqThrottle = pedalTimeout ? 0u : (havePedal ? clamp_0_100((int)pedalPctTmp) : s_safety.lastSafe.throttle_pct);
