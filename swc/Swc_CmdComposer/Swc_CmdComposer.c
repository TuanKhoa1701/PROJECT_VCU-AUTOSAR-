 ...
 *
 * @version 1.0
 * @date    2025-09-10 (Asia/Ho_Chi_Minh)
 * @author  HALA
 **********************************************************/
...
typedef struct {
  uint8   lastThrottle;  /* 0..100 */
  uint8   lastGearU8;    /* 0..3   */
  uint8   lastModeU8;    /* 0..1   */
  boolean lastBrake;     /* TRUE/FALSE */
  uint8   aliveNibble;   /* 0..15 */
  boolean inited;
} CmdComposer_State_t;
...
static uint8 clamp_0_100(int v)
{
  if (v < 0)   return 0u;
...
static uint8 gear_to_u8(Gear_e g)
{
  switch (g) {
...
static uint8 mode_to_u8(DriveMode_e m)
{
  switch (m) {
...
static uint8 rate_limit_u8(uint8 target, uint8 current, uint8 step)
{
  if (step == 0u) return target;
  if (target > current) {
    uint8 inc = (uint8)((target - current) > step ? step : (target - current));
    return (uint8)(current + inc);
  } else if (target < current) {
    uint8 dec = (uint8)((current - target) > step ? step : (current - target));
    return (uint8)(current - dec);
  }
  return current;
}
...
void Swc_CmdComposer_Run10ms(void)
{
  ...
  uint8  thr  = haveSafe ? clamp_0_100((int)safe.throttle_pct) : s_cmd.lastThrottle;
  uint8  gear = haveSafe ? gear_to_u8(safe.gear)               : s_cmd.lastGearU8;
  uint8  mode = haveSafe ? mode_to_u8(safe.driveMode)          : s_cmd.lastModeU8;
  ...
  s_cmd.aliveNibble = (uint8)((s_cmd.aliveNibble + 1u) & 0x0Fu);
  (void)Rte_Write_CmdComposer_VcuCmdTx_AliveCounter(s_cmd.aliveNibble);
