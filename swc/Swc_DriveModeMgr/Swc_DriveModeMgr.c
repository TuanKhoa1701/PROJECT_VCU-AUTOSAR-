...
#define DRIVEMODE_DEBOUNCE_MS      (30u)  /* yêu cầu ổn định 30 ms */
#endif

/* Số mẫu cần liên tiếp giống nhau để coi là ổn định */
#define DRIVEMODE_DEBOUNCE_TICKS \
  ((uint8)((DRIVEMODE_DEBOUNCE_MS + (DRIVEMODE_TASK_PERIOD_MS - 1u)) / DRIVEMODE_TASK_PERIOD_MS))
...
typedef struct {
  ...
  uint8       cnt;      /* số lần liên tiếp cùng giá trị   */
