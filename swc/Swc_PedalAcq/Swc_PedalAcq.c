...
/* Trạng thái nội bộ */
typedef struct {
  uint16 filt_q;     /* giá trị lọc dạng QK (x4 nếu K=2)         */
  uint8  outPct;     /* % đã publish lần gần nhất (0..100)       */
...
static void PedalAcq_SeedFromHw(void)
{
  ...
  s_pedal.filt_q = ((uint16)raw) << PEDAL_EMA_SHIFT_K;
  s_pedal.outPct = raw;
  s_pedal.inited = TRUE;
...
void Swc_PedalAcq_Run10ms(void)
{
  ...
  uint16 raw_q = ((uint16)raw) << PEDAL_EMA_SHIFT_K;
  s_pedal.filt_q = (uint16)(s_pedal.filt_q + ((raw_q - s_pedal.filt_q) >> PEDAL_EMA_SHIFT_K));

  /* 3) Tính mục tiêu nguyên % từ filt_q */
  uint8 target = (uint8)(s_pedal.filt_q >> PEDAL_EMA_SHIFT_K);
